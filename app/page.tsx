...
export default function AdminChat() {
  ...
  const loadConversation = async (conversationId: string) => {
    ...
    if (currentConversationId && currentConversationId !== conversationId && githubToken && userId) {
      ...
      try {
        const { ConversationMemoryService } = await import('@/app/services/conversation-memory.service');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      try {
        ...
        const { VectorSearchService } = await import('@/app/services/vector-search.service');
      ...
      let systemMessageWithContext = systemMessage || 'You are a helpful AI assistant. You have access to web search and document knowledge base. Use these tools to provide accurate, up-to-date information. Always search for current information when needed.';
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70">
          <div className={`rounded-lg shadow-xl p-6 w-full max-w-md ${isDarkMode ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
            <h2 className={`text-2xl font-bold mb-4 ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Login to Allerac-One</h2>
            {loginError && (
            ...
            <div className="max-w-3xl mx-auto px-4 py-8">
              {messages.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <p className={isDarkMode ? 'text-gray-400' : 'text-gray-500'}>Ask me anything or use web search to find current information.</p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-8 max-w-2xl mx-auto">
                    <button
                      onClick={() => setInputMessage("What are the latest developments in AI technology?")}
                    ...
                    >
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>AI Technology</p>
                      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Get latest AI news</p>
                    </button>
                    <button
                      onClick={() => setInputMessage("Explain quantum computing in simple terms")}
                      className={`p-4 rounded-xl border transition-all text-left ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-blue-500' : 'bg-white border-gray-200 hover:border-blue-500 hover:shadow-md'}`}
                    >
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Quantum Computing</p>
                      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Simplified explanation</p>
                    </button>
                    <button
                      onClick={() => setInputMessage("What's the weather forecast for this week?")}
                      className={`p-4 rounded-xl border transition-all text-left ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-blue-500' : 'bg-white border-gray-200 hover:border-blue-500 hover:shadow-md'}`}
                    >
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Weather Forecast</p>
                      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Check current weather</p>
                    </button>
                    <button
                      onClick={() => setInputMessage("Recommend a good book to read")}
                      className={`p-4 rounded-xl border transition-all text-left ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:border-blue-500' : 'bg-white border-gray-200 hover:border-blue-500 hover:shadow-md'}`}
                    >
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Book Recommendation</p>
                      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Get reading suggestions</p>
                    </button>
                  </div>
                </div>
              )}
          ...
          <div className={`backdrop-blur-md rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col ${isDarkMode ? 'bg-gray-800/95 border border-gray-700' : 'bg-white/95 border border-gray-200'}`}>
            <div className={`p-6 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              ...
              <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Configure the global memory that will be shared across all conversations and all users.
                This helps the AI remember important context and instructions.
              </p>
            </div>
            <div className="p-6 flex-1 overflow-y-auto">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Enter information and instructions that the AI should remember..."
                  rows={12}
                ...
                />
                <p className={`text-xs mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  💡 Include important context, special instructions, preferences, or any information 
                  that should be available to the AI in every conversation.
                </p>
              </div>
            </div>
