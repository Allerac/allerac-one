...
describe('Control API v1 health proxy', () => {
  ...
  describe('GET /proxy/status', () => {
    ...
    it('reports connected status', async () => {
      mockPool.query.mockResolvedValueOnce({
        rows: [{ is_connected: true, mfa_pending: false, sync_enabled: true, last_sync_at: null, last_error: null }],
      } as any);
  ...
  describe('GET /proxy/activities', () => {
    ...
    it('fetches activities live from the worker and never touches health_activities', async () => {
      mockPool.query.mockResolvedValueOnce({
        rows: [{ oauth1_token_encrypted: 'encrypted', is_connected: true }],
      } as any);
      ...
      // One read (garmin_credentials) and nothing else — no INSERT/UPDATE
      // into health_activities for this request.
      expect(mockPool.query).toHaveBeenCalledTimes(1);
      expect(mockPool.query).toHaveBeenCalledWith(expect.stringContaining('garmin_credentials'), [user.id]);
  ...
  describe('GET /proxy/daily', () => {
    ...
    it('returns 409 when garmin is not connected', async () => {
      mockPool.query.mockResolvedValueOnce({ rows: [{ is_connected: false }] } as any);
