...
// pick which model answers on its behalf. Same pattern as /sales — see
...
function ModelPicker({ isDark }: { isDark: boolean }) {
  ...
  useEffect(() => {
    void domainActions.getDomainModelSettings('openworld').then(setSettings);
  }, []);
    ...
    <div className={`p-4 rounded-lg border ${isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          ...
          <p className={`text-xs ${muted}`}>Which model answers in the OpenWorld FAQ chat.</p>
        </div>
      </div>
