 ...
 *   AZURE_STORAGE_CONTAINER_NAME - Name of blob container (e.g., social-posts)
 *
 * Local development:
...
export class AzureStorageUploadService implements ImageUploadService {
  ...
  async upload(buffer: Buffer, filename?: string, directory?: string): Promise<UploadedImage> {
    try {
      const containerClient = this.blobServiceClient.getContainerClient(this.containerName);

      // Generate unique blob name: timestamp + original filename
      const timestamp = Date.now();
      const safeDirectory = directory?.trim().replace(/^\/+|\/+$/g, '').replace(/[^a-zA-Z0-9/_-]/g, '');
      const fileName = `${timestamp}-${filename || 'image.jpg'}`;
      const blobName = safeDirectory ? `${safeDirectory}/${fileName}` : fileName;
