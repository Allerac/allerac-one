...
import MusicDashboard from '@/app/components/music/MusicDashboard';
...
export default function MusicClient({ userId, userName, userEmail, isAdmin, defaultSkillName }: Props) {
  ...
  const {
    ...
    userId, domain: 'music', defaultSkillName,
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`flex-1 flex flex-col overflow-hidden ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              {/* Music dashboard — main panel */}
              <div className={`${mobileMusicTab === 'dashboard' ? 'flex' : 'hidden'} lg:flex flex-1 flex-col overflow-hidden`}>
                <MusicDashboard
                  isDarkMode={d}
                  onViewChange={(context) => setPostContext(context)}
                />
              </div>

              {/* Chat panel */}
              <div className={`${mobileMusicTab === 'chat' ? 'flex flex-1' : 'hidden'} lg:flex lg:flex-none lg:w-[360px] flex-col border-l overflow-hidden ${d ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-white'}`}>
                  ...
                  <div className={`flex-1 flex flex-col items-center justify-center px-4 ${d ? 'bg-gray-900' : 'bg-white'}`}>
                    <div className="w-full max-w-sm">
                      <div className="text-center mb-8">
                        ...
                        <h3 className={`text-sm font-medium ${d ? 'text-gray-400' : 'text-gray-600'}`}>What should you listen to today?</h3>
                      </div>
                      <ChatInput
                  ...
                  <>
                    <div className={`flex-1 overflow-y-auto ${d ? 'bg-gray-900' : 'bg-white'}`}>
                      <ChatMessages
                        ...
                        messagesEndRef={messagesEndRef} domainSlug="music"
                      />
                    </div>
