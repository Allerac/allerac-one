/**
 * GET /api/tax-filings/[id]/file — streams the attached filing proof PDF/image back.
 *
 * Auth: admin session.
 */
...
export async function GET(
  ...
  if (!UUID_PATTERN.test(id)) return new Response('Invalid filing ID', { status: 400 });

  const result = await pool.query(
    'SELECT file_name, file_type, file_data FROM tax_filings WHERE id = $1',
    [id]
  );
  ...
  return new Response(row.file_data, {
    headers: {
      ...
      'Content-Disposition': `inline; filename="${(row.file_name || 'filing').replace(/"/g, '')}"`,
