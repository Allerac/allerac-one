...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const res = date
      ? await pool.query(
          `SELECT ${ACTIVITY_DETAIL_COLUMNS} FROM health_activities WHERE user_id = $1 AND date = $2 ORDER BY start_time_seconds DESC LIMIT $3`,
          [user.id, date, limit],
        )
      : await pool.query(
          `SELECT ${ACTIVITY_DETAIL_COLUMNS} FROM health_activities WHERE user_id = $1 ORDER BY date DESC, start_time_seconds DESC LIMIT $2`,
          [user.id, limit],
        );
