...
export function AlleracIcon({ size = 16, className = '' }: AlleracIconProps) {
  // Gradient/filter ids must be unique per instance — multiple AlleracIcons
  // can render on the same page (e.g. notes: vault empty-state + chat
  // greeting), and duplicate SVG ids break url(#...) references in the
  // second instance, leaving it as a blank white circle.
  const uid = useId();
  const gradId = `allerac-icon-grad-${uid}`;
  const shadowId = `allerac-icon-shadow-${uid}`;
