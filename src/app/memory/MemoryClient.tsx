...
export default function MemoryClient({ userId, userName, userEmail, isAdmin, defaultSkillName }: Props) {
  const { isDark: isDarkMode, toggleDark } = useTheme();
  const [isSidebarCollapsed, setSidebarCollapsed] = useState(true);
  const [isSidebarOpen, setSidebarOpen]       = useState(false);
  const [postContext, setPostContext]         = useState('');
  const [mobileTab, setMobileTab]             = useState<'knowledge' | 'chat'>('knowledge');
  ...
  } = useConversations(userId, 'memory');
  ...
  const {
    ...
    userId, domain: 'memory', defaultSkillName,
  ...
  return (
    <DomainProvider value={{ isDark: d, lastToolCall, setLastToolCall, postContext, setPostContext }}>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`flex-1 flex flex-col overflow-hidden ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              {/* Knowledge panel — full screen on mobile when knowledge tab, flex-1 on desktop */}
              <div className={`${mobileTab === 'knowledge' ? 'flex flex-1' : 'hidden'} lg:flex lg:flex-1 overflow-hidden`}>
                <MemoryGraphPanel />
              </div>
                    ...
                    <div className="w-full max-w-lg">
                      <div className="text-center mb-8">
                        ...
                        <h3 className={`text-sm font-medium ${d ? 'text-gray-400' : 'text-gray-600'}`}>
                          Ask about your memories, documents, or what Allerac knows about a topic.
                        </h3>
                      </div>
                      <ChatInput
                  ...
                  <>
                    <div className={`flex-1 overflow-y-auto ${d ? 'bg-gray-900' : 'bg-white'}`}>
                      <ChatMessages
                        ...
                        messagesEndRef={messagesEndRef} domainSlug="memory"
                      />
                    </div>
      ...
      <MyAlleracModal
        ...
        userName={userName ?? undefined} domainSlug="memory"
      />
    </DomainProvider>
  );
}
