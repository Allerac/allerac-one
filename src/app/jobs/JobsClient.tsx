...
export default function JobsClient({ userId, userName, userEmail, isAdmin, defaultSkillName }: Props) {
  ...
  const [mobileTab, setMobileTab]                 = useState<'jobs' | 'chat'>('jobs');
    ...
    <DomainProvider value={{ isDark: d, lastToolCall, setLastToolCall, postContext: '', setPostContext: () => {} }}>
      <div className={`h-full flex flex-col ${d ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className={`flex-1 flex flex-col overflow-hidden ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              {/* Jobs panel */}
              <div className={`${mobileTab === 'jobs' ? 'flex flex-1' : 'hidden'} lg:flex lg:flex-1 flex-col overflow-hidden`}>
                  ...
                  <div className={`flex-1 flex flex-col items-center justify-center px-4 ${d ? 'bg-gray-900' : 'bg-white'}`}>
                    <div className="w-full max-w-sm">
                      <div className="text-center mb-8">
                        ...
                        <h3 className={`text-sm font-medium ${d ? 'text-gray-400' : 'text-gray-600'}`}>Ask me about your scheduled jobs.</h3>
                      </div>
                      <ChatInput
                  ...
                  <>
                    <div className={`flex-1 overflow-y-auto ${d ? 'bg-gray-900' : 'bg-white'}`}>
                      <ChatMessages
                        ...
                        messagesEndRef={messagesEndRef} domainSlug="jobs"
                      />
                    </div>
        ...
        <MyAlleracModal
          ...
          userName={userName ?? undefined} domainSlug="jobs"
        />
      </div>
    </DomainProvider>
  );
